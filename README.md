...
| **国标公差** | 未注公差自动判定 | GB/T 1804-m 未注线性公差 + GB/T 1184-K 几何公差 |
| 图纸清单校验 | 按检验标准逐项判定 | 表面类视觉 + 尺寸类实测值量化判定 |

### 双端界面

- **检验员端**：登记批次（支持扫码枪）→ **图纸查阅（按料号自动关联）** → 检测模式（标准件/加工件/螺纹）→ 引擎选择 → 拍照规范指引 + 图像质检 → 检测 → 入库 → 打印检测单 → 复核
- **主管端**：质量总览（判定/缺陷/良率趋势/复核一致性/抽检覆盖率）、物料白名单、**图纸管理（上传 PDF/图片）**、检验标准编辑器（含国标公差）、追溯查询、**上机反查**、数据积累仪表盘
...
3. **国标公差知识库**：GB/T 1804 未注公差 + GB/T 1184 几何公差按尺寸段自动查表，图纸未标公差的尺寸也能自动判定
4. **本地免费引擎**：规格/缺陷/螺纹/尺寸全走本地模型（RTX 4060），视觉大模型仅疑难件可选
5. **检验标准结构化**：检验部位 + 图纸阈值 + 量化判定，机器判定覆盖视觉 UNSURE
6. **质量数据资产**：批次/供应商/料号全程留痕，复核回灌 → 本地模型越用越准；上机反查 + 抽检覆盖率统计定位漏检
7. **AI 初筛 + 工程师决策**：不是替代检验员，而是降低负担、提高一致性
