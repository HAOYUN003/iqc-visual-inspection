...
from detection import spec_model, defect_model
...
def load_models():
    ...
    if _DEFECT["model"] is None:
        try:
            _DEFECT["model"] = defect_model.load_defect_model(device)
        except Exception as e:
            print(f"[pipeline] 缺陷模型加载失败: {e}")
    return device
...
def run_detection(img_bgr, material_no=None, inspector="", save_image=True):
    ...
    返回 dict：
        ...
        defect_summary, defect_boxes, annotated, ai_verdict,
        reasons
    """
    import cv2
    load_models()
    ...
    # 2) 缺陷检测（加工件）
    if _DEFECT["model"] is not None:
        summary, boxes, annotated = defect_model.predict_defects(
            _DEFECT["model"], img_bgr, conf_thresh=DEFECT_CONF_THRESH)
